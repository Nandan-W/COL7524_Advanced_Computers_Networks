...
int main (int argc, char *argv[])
{
  ...
  p2p14.SetDeviceAttribute ("DataRate" , StringValue("12Mbps"));
  ...
  // Static Route to AS2 from AS1 
  Ipv4StaticRoutingHelper staticRoutingHelper;
  Ptr<Ipv4> ipv4AS1_static = routers.Get(0) -> GetObject<Ipv4> ();
  Ptr<Ipv4StaticRouting> staticRoutingAs1 = staticRoutingHelper.GetStaticRouting(ipv4AS1_static);

  staticRoutingAs1 -> AddHostRouteTo(Ipv4Address("192.168.3.2"), i12.GetAddress(1),1);

  // static route to AS3 from AS2
  Ptr<Ipv4> ipv4AS2_static = routers.Get(1) -> GetObject<Ipv4> ();
  Ptr<Ipv4StaticRouting> staticRoutingAs2 = staticRoutingHelper.GetStaticRouting(ipv4AS2_static);

  staticRoutingAs2 -> AddHostRouteTo(Ipv4Address("192.168.3.2"), i23.GetAddress(1), 2);
  ...
  //animation interface
  AnimationInterface anim("interas-baseline-anim-partC-method2.xml");
  anim.SetConstantPosition(routers.Get(0), 0.0 ,0.0);
  anim.SetConstantPosition(routers.Get(1), 0.0 ,20.0);
  anim.SetConstantPosition(routers.Get(2), 60.0 ,20.0);
  anim.SetConstantPosition(routers.Get(3), 60.0 ,0.0);

  //node labels
  anim.UpdateNodeDescription(routers.Get(0), "AS1");
  anim.UpdateNodeDescription(routers.Get(1), "AS2");
  anim.UpdateNodeDescription(routers.Get(2), "AS3");
  anim.UpdateNodeDescription(routers.Get(3), "AS4");

  anim.UpdateNodeColor(routers.Get(0), 0, 255, 0);
  anim.UpdateNodeColor(routers.Get(3), 255, 0, 0);
  ...
  monitor->SerializeToXmlFile ("interas-baseline-results_partC_method2.xml", true, true);
