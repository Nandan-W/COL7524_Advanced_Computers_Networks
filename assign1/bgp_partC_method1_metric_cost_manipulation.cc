...
int main (int argc, char *argv[])
{
  ...
  // setting path from AS1 to AS2 by interface metric/cost adjustment 
  Ptr<Ipv4> ipv4AS1 = routers.Get(0) -> GetObject<Ipv4>();
  uint32_t ifIndexToAS4 = ipv4AS1 -> GetInterfaceForDevice(d14.Get(0));
  ipv4AS1 -> SetMetric(ifIndexToAS4,10);
  //path hop cost of as1->as4 link to 10 compared to 3 for as1-as2-as3-as4 

  Ptr<Ipv4> ipv4AS4 = routers.Get(3) -> GetObject<Ipv4>();
  uint32_t ifIndexToAS1 = ipv4AS4 -> GetInterfaceForDevice(d14.Get(1));
  ipv4AS4 -> SetMetric(ifIndexToAS1, 10);
  //path hop cost of as4->as1 link to 10 compared to 3 for as1-as2-as3-as4 
  ...
  //animation interface
  AnimationInterface anim("interas-baseline-anim-partC-method1.xml");
  ...
  monitor->SerializeToXmlFile ("interas-baseline-results_partC_method1.xml", true, true);
